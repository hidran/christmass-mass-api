...
export class FedeleService {

// lettura dati da server laraapi
fedeli: Fedele[] = [];  // definisco i dati come array vuoto

private prefixTable = "/fedele";
...
// la chiamata la faccio solo se ho il token per abilitare la lettura solo a uteti loggati
    ...
    deleteFedeli(fedele: Fedele) {
      return this.http.delete(this.APIURL + '/' + fedele.id);

    }



updateFedele(fedele: Fedele) {
  ...
  return this.http.patch(this.APIURL + '/' + fedele.id, fedele);

}


 createFedele(fedele: Fedele){
  return this.http.post(this.APIURL, fedele);
}
